...
func (s *sanitizedError) As(target any) bool {
    ...
    // We want to completely block As delegating to s.err if the target is an interface type
    // that url.Error implements, EXCEPT net.Error which we handled.
    // Let's see what happens if we just don't delegate to errors.As(s.err, target) at all.
    // Or if we specifically extract the underlying error of url.Error.
...
func main() {
    ...
    if errors.As(err, &t) {
        ...
        fmt.Println("Could not extract unwrapInterface")
    }
}
